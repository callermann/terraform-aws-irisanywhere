...
data "template_file" "iris_policy_base" {
  ...
  vars = {
    cluster = replace("${var.hostname_prefix}-${var.deployment_name != "" ? var.deployment_name : var.instance_type}", ".", "")
  }
}
...
resource "aws_iam_role" "iris" {
  name               = var.iam_role_name != "" ? var.iam_role_name : replace("${var.hostname_prefix}-${var.deployment_name != "" ? var.deployment_name : var.instance_type}-Role", ".", "")
  assume_role_policy = data.template_file.iris_role.rendered
}

resource "aws_iam_instance_profile" "iris" {
  name = replace("${var.hostname_prefix}-${var.deployment_name != "" ? var.deployment_name : var.instance_type}-Profile", ".", "")
  role = aws_iam_role.iris.name
}

resource "aws_iam_policy" "iris_policy_base" {
  name   = replace("${var.hostname_prefix}-${vvar.deployment_name != "" ? var.deployment_name : var.instance_type}-Policy", ".", "")
...
resource "aws_iam_policy" "iris_combined" {
  name   = replace("${var.hostname_prefix}-${var.deployment_name != "" ? var.deployment_name : var.instance_type}-IAM-Policy", ".", "")
